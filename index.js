...
app.post('/size2json/', upload.single('image'), async (req, res) => {
    ...
    if (!req.file) {
        return res.status(400).json({ 
            error: 'Изображение не найдено. Отправьте файл в поле "image"' 
        });
    }
    ...
    try {
        ...
        res.status(400).json({ 
            error: 'Не удалось определить размеры. Убедитесь, что файл является PNG-изображением' 
        });
    }
});
...
app.listen(port, '0.0.0.0', () => {
    ...
    console.log(` Маршрут /login/ готов`);
    console.log(` Маршрут /size2json/ готов`);
});